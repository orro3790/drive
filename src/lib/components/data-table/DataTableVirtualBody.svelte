<!--
@component DataTableVirtualBody
Renders a virtualized table body for large datasets.
Uses padding-based virtualization (not absolute positioning) for simplicity.
...
<script lang="ts" generics="RowType">
	...
	let containerHeight = $state(400); // fallback
	...
	$effect(() => {
		...
		containerHeight = scrollContainerRef.clientHeight;
		...
		// Also track resize for container height changes
	...
	// Get all rows (includes expanded children)
	...
	// Derive column visibility state for row key
	...
	// Column sizing state (used for width application and to detect user resizing)
	...
	// Derive column pinning state for reactivity (referenced in getPinnedOffset via cell.column.getIsPinned())
	...
	const visibleCount = $derived(Math.ceil(containerHeight / rowHeight) + overscan * 2);

	const endIndex = $derived(Math.min(startIndex + visibleCount, allRows.length));

	// Slice to visible rows only
	const visibleRows = $derived(allRows.slice(startIndex, endIndex));

	// Padding to maintain scroll height
	const paddingTop = $derived(startIndex * rowHeight);
	const paddingBottom = $derived(Math.max(0, (allRows.length - endIndex) * rowHeight));

	// ----- Row helpers (same as DataTableBody) -----
	...
	function getRowIndex(globalIndex: number): number {
		return currentPage * pageSize + globalIndex + 1;
	}
	...
	function canSelectRow(rowApi: (typeof allRows)[number]): boolean {
...
<tbody class="virtual-tbody">
	...
	{#if paddingTop > 0}
		<tr class="spacer-row" aria-hidden="true">
			<td style:height="{paddingTop}px" colspan="100"></td>
		</tr>
	{/if}
	{#each visibleRows as row, localIndex (`${row.id}-${expandedState[row.id] ?? false}-${selectionState[row.id] ?? false}-${visibleColumnsKey}-${columnPinningKey}`)}
		{@const globalIndex = startIndex + localIndex}
		{@const rowData = row.original}
		{@const extraClass = rowClass?.(rowData, globalIndex) ?? ''}
		...
		>
					...
					<div class="cell-center" use:stopRowClick>
						{#if rowCanSelect}
							<Checkbox
								...
								ariaLabel={m.table_row_select({ index: getRowIndex(globalIndex).toString() })}
			...
			{/if}
			{#if showIndex}
				<td class="col-index">{getRowIndex(globalIndex)}</td>
			{/if}
	...
	{#if paddingBottom > 0}
		<tr class="spacer-row" aria-hidden="true">
			<td style:height="{paddingBottom}px" colspan="100"></td>
		</tr>
	{/if}
</tbody>
...
<style>
	...
	.cell-center {
		...
		width: 100%;
		height: 100%;
		min-height: var(--data-table-row-min-height, 40px);
	}
